...
function ImageFormatter({ memeRef, header, footer, imageUrl, headerColor, footerColor }) {
  return (
    <Fragment>
      ...
      <div className={styles.container}>
        <div className={styles.meme} ref={memeRef}>
          <p style={{ color: headerColor }} className={styles.header}>{header}</p>
          <img src={imageUrl} className={styles.img}/>
          <p style={{ color: footerColor }} className={styles.footer}>{footer}</p>
        </div>
      </div>
    </Fragment>
  );
